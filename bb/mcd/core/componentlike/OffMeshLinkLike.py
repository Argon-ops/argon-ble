...
from bb.mcd.core.componentlike.util import UnderscoreUtils
...
def _getCamelKeys():
    """Returns camel case equivalents of our suffix keys. (Used only to save some work in the Display function) """
    global _camelKeys
    if not _camelKeys or len(_camelKeys) == 0:
        _camelKeys = [UnderscoreUtils.underscoreToCamel(k)
                      for k in _suffixes.keys() if k not in _exclude]
    return _camelKeys
    ...
    def OnRemoveKey(key: str, targets):
        for suffix in _suffixes.keys():
            AbstractDefaultSetter._RemoveKey(
                _getSuffixKey(suffix), targets=targets)
...
def register():
    ...
    bpy.types.Scene.offMeshLinkLike = bpy.props.PointerProperty(
        type=OffMeshLinkLike)
