...
from bb.mcd.core.command import CommandNameItems
...
class ASSharedDataUtil:
    ...
    def GetCommandDataSharedObject():
        ob = SharedDataObject.getSharedDataObjectWithName(
            "z_DuksGames_AS_SharedObject")
        # tag object so that the importer will see it
        ob["mel_action_starter"] = 1
        return ob
...
class CUSTOM_PG_AS_Collection(PropertyGroup):
    ...
    def ToDict(pgSelf):
        ...
        for fieldName in pgSelf.__annotations__.keys():
            serVal = CUSTOM_PG_AS_Collection.GetSerialiazableValue(
                pgSelf, fieldName)
            d[fieldName] = serVal
        return d
    ...
    commandBehaviourType: EnumProperty(
        ...
        items=(
            ('RestartForwards', 'RestartForwards',
             'Just restart play from the beginning.'),
            ('ToggleAndRestart', 'ToggleAndRestart',
             'If the playback cursor is closer to the end, play backwards starting from the end. Else play forwards from the beginning.'),
            ('FlipDirections', 'FlipDirections',
             'Switch between forward and reverse with each invocation. Just change directions and don\'t set the playback position (to either start or end) before hand.'),
            ('RestartBackwards', 'RestartBackwards',
             'Restart play from the end and play to the beginning'),
        ),
    )
    ...
    overTimeFunction: EnumProperty(
        items=(
            ('StartValueEndValue', 'StartValueEndValue',
             'Send the low value immediately. Send the high value after the Duration has elapsed'),
            ('SawTooth', 'SawTooth',
             'Function f(t) = (t % period) / period * (high - low) + high'),
...
f(t) = t*(highValue-lowValue)/periodSeconds + lowValue. So f(periodSeconds) = highValue",
    ...
    outroBehaviour: EnumProperty(
        items=(
            ...
            ('ThresholdCondition', 'ThresholdCondition',
             'Outro plays to destination A if the signal meets the threshold (e.g. animation reaches beyond halfway point). Plays to B otherwise')
        )
    )
    ...
    playAfter: EnumProperty(
        ...
        set=lambda self, value: CLU.storePlayableName(
            self, value, "playAfterStor")  # self.setPlayAfter(value)
    )
    ...
    def draw(self, layout):
        ...
        split.operator(CU_OT_PlayablePickPopup.bl_idname, text="",
                       icon="GREASEPENCIL").playableName = self.name
...
class CU_OT_PlayablePickPopup(bpy.types.Operator):
    ...
    def drawTargetsList(self, playable, box):
        from bb.mcd.cduoperator.InspectorPopup import CU_OT_InspectorPopup
        rowb = box.row()
        ...
        for targ in playable.targets:
            ...
            rowb.operator(CU_OT_Select.bl_idname, icon='RESTRICT_SELECT_OFF',
                          text="").target = targ.object.name if targ.object else ""
            rowb.operator(AS_OT_RemoveFromTargets.bl_idname, icon='X',
                          text="").playableNameConcatTargetName = F"{playable.name},,,{targ.name}"
            if targ.object:
                rowb.operator(CU_OT_InspectorPopup.bl_idname, text="Edit",
                              icon="KEYTYPE_JITTER_VEC").objectName = targ.object.name if targ.object else ""

        rowb = box.row()
        rowb.operator(AS_OT_AddToTargets.bl_idname, icon='ADD',
                      text="ADD").playableName = playable.name  # self.playableName
    ...
    def draw(self, context):
        ...
        if playable is None:
            print(
                F"Something went wrong in Playable popup draw function. with playableId: [{self.playableId}]")
            return
        ...
        if playableType == 0:  # event only
            ...
            if playable.overTime:
                ...
                if playable.outroBehaviour != "None":
                    ...
                    if playable.outroBehaviour == "ThresholdCondition":
                        ...
                        boxx.row().prop(playable, "outroDestinationValueB",
                                        text="Under Threshold Destination")
            ...
            self.layout.row().prop(playable, "shakeDisplacementDistance",
                                   text="Displacement Distance")
        ...
        if playable.shouldPlayAfter:
            ...
            print(
                F"Display play after now: {playable.playAfter} | stor: {playable.playAfterStor}")
            row.operator(CU_OT_PlayablePickPopup.bl_idname, text="",
                         icon="GREASEPENCIL").playableName = playable.playAfter
            self.layout.row().prop(playable, "playAfterAdditionalDelay", text="Delay Seconds")
            self.layout.row().prop(playable, "playAfterDeferToLatest",
